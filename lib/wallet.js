...
module.exports = (duniterServer, payperblock) => {
  ...
  return {
    ...
    topGifts: () => co(function*() {
      const sources = yield duniterServer.dal.sindexDAL.query('SELECT * ' +
        'FROM s_index s, txs t ' +
        'WHERE s.tx = t.hash ' +
        'AND op = ? ' +
        'AND conditions = ? ' +
        'AND t.issuers NOT LIKE ?' +
        'ORDER BY CAST(written_on as int) DESC ' +
        'LIMIT 5', ['CREATE', 'SIG(' + remuniterPubkey + ')', '["' + remuniterPubkey + '"]']);
      const enriched = yield sources.map((src) => co(function*() {
        const txs = yield duniterServer.dal.txsDAL.query('SELECT * FROM txs WHERE hash = ?', [src.identifier]);
        let issuer = 'unknown';
        let date = '----------';
        let amount = src.amount * Math.pow(10, src.base);
        if (txs && txs.length === 1) {
          const tx = txs[0];
          const block = yield duniterServer.dal.getBlock(tx.block_number);
          date = moment(block.medianTime * 1000).format('YYYY-MM-DD hh:mm');
