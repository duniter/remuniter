    ...
    return co(function *() {
      ...
      for (let i = 0, len = issuers.length; i < len; i++) {
        ...
        if (blocksCount == current.issuersFrame) {
          ...
          // Format handicap and exclusionFactor
          if (stats[issuers[i]].handicap == 0) { stats[issuers[i]].handicap = '-'; }
          if (stats[issuers[i]].exclusionFactor == 1) { stats[issuers[i]].exclusionFactor = '-'; }
        }
      }
