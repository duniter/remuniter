  ...
  function getWhatWasPaid(pubkey, txh, blocks) {
    return txh.sent.reduce((sum, tx) => {
      ...
      tx.outputs.forEach((out) => {
        let match = (out.raw || out).match(/(\d+):(\d+):SIG\((.*)\)/);
        if (match) {
    ...
    return co(function *() {
      ...
      for (let i = 0, len = issuers.length; i < len; i++) {
        ...
        if (blocksCount == current.issuersFrame) {
          ...
          maxDiff = (maxDiff<stats[issuers[i]].diff) ? stats[issuers[i]].diff:maxDiff;
          maxExclusionFactor = (maxExclusionFactor<stats[issuers[i]].exclusionFactor) ? stats[issuers[i]].exclusionFactor:maxExclusionFactor;
          maxHandicap = (maxHandicap<stats[issuers[i]].handicap) ? stats[issuers[i]].handicap:maxHandicap;
        }
      }
  ...
  return {
    ...
    pay: (fromBlock, toBlock) => co(function *() {
      ...
      if (availableMoney >= reallyPaid) {
        ...
        // ----- SUBMITTING -----
        yield duniterServer.writeRawTransaction(getRawTransaction(tx));
