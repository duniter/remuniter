...
module.exports = (host, port, homeFolderName, httpLogs) => co(function *() {
  ...
  let pay = () => co(function *() {
    try {
      ...
      if (lastCurrent && current.number >= lastCurrent.number + 0) {
        yield duniterServer.dal.txsDAL.exec('DELETE FROM txs WHERE NOT written AND issuers like \'%' + remuWallet.pubkey + '%\'');
      }
      lastCurrent = current;
